...
def main() -> None:
    ...
    evidence_state = registry + (
        ...
        '    private static final class Af8EffectEvidence {final Set<Integer> ids=new LinkedHashSet<>();final List<Card> targets=new ArrayList<>();String kind="",expected="",error="";int positive;}\n'
    ...
    run = r'''private static void runCampaign(Game game,List<CaseSpec>cases,Map<String,CaseEvidence>evidence,AtomicReference<String>currentPath){
        ws33CaseSpecs.clear();ws33Af8Effects.clear();ws33Af8Observations.clear();for(CaseSpec c:cases)ws33CaseSpecs.put(c.pathId,c);
    ...
    required = (
        ...
        'pathSpec.script.contains("Announce$ X")',
    )
