#!/usr/bin/env python3
"""WS33-D D1 filter: bind the prepared case set to the authoritative queue.

Verifies the queue item (WS33D/template-061/STATE_ONLY/LifeLoseEffect),
...
def main() -> int:
    ...
    rows = [x for x in queue["items"] if x.get("logical_bucket") == "WS33D"
            and x.get("scenario_group_id") == "ws33-g2-template-061"
            and x.get("evidence_profile") == "STATE_ONLY"
            and x.get("runtime_subsystem") == "forge.game.ability.effects.LifeLoseEffect"]
    ...
    assert len(case_ids) == 8 and len(set(case_ids)) == 8
    ...
    print(f"WS33_ABC_D2_FILTER=PASS cases=8 deferred=10 digest={digest}")
    return 0
