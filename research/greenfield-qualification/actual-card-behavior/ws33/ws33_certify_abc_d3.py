#!/usr/bin/env python3
"""WS33-D D3 certifier: adjudicate the damageall campaign evidence gate.
...
Emits the D3 gate JSON. Any violation fails closed (nonzero exit).
"""
...
def main() -> int:
    ...
    assert plan["provable_count"] == 7 and len(plan["provable_ids"]) == 7
    deferred = json.loads(a.deferred.read_text())
    assert len(deferred) == 10
    root = a.campaign_root

    for name in ("damageall-record-diagnostics.jsonl", "damageall-replay-diagnostics.jsonl"):
        p = root / name
        assert p.is_file() and p.stat().st_size == 0, f"diagnostics not empty: {name}"
    index = json.loads((root / "campaign-index.json").read_text())
    assert len(index["records"]) == 7, "campaign index must admit 7 records"
    ...
    gate = {
        "schema": "commander-simulator-next.ws33d-d3-gate.v1",
        "batch": "D3",
    ...
    print(f"WS33_ABC_D3_CERTIFY=PASS paths=7 decisions={decision_events} rng={rng_events}")
    return 0
