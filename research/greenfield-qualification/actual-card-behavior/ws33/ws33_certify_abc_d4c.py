#!/usr/bin/env python3
"""WS33-D D4c certifier: adjudicate the single-target campaign evidence gate.

Verifies markers, campaign index, empty diagnostics, per-record assertion
results (including the measured single-TARGET_SELECTION designated-target
proof, command-zone Effect assertions, and zone/counter assertions),
decision-tape ACCEPTED-only validation with exactly one TARGET_SELECTION
event per record, RNG replay service, hidden-observation PASS, replay
divergence 0, and source/pin/model binding. Emits the D4c gate JSON. Any
violation fails closed (nonzero exit).
"""
...
def main() -> int:
    ...
    assert plan["provable_count"] == 4 and len(plan["provable_ids"]) == 4
    assert plan.get("batch") == "D4c"
    intents = plan.get("intents", {})
    assert len(intents) == 4 and all(v.startswith("ENTITY:") for v in intents.values()), \
        "D4c-T1 requires ENTITY intents on all 4 paths"
    ...
    for name in ("target-record-diagnostics.jsonl", "target-replay-diagnostics.jsonl"):
        p = root / name
        assert p.is_file() and p.stat().st_size == 0, f"diagnostics not empty: {name}"
    index = json.loads((root / "campaign-index.json").read_text())
    assert len(index["records"]) == 4, "campaign index must admit 4 records"
    ...
    for pid in plan["provable_ids"]:
        ...
        assert "target-designated-chosen" in assertion_ids, "missing designated-target proof assertion"
        ...
        assert trace.get("effect_kind") in ("static", "trigger", "replacement"), "missing effect kind"
        assert trace.get("target_designated_chosen") is True, "trace designated-target proof must read true"
        tape = json.loads((d / "decision-tape.json").read_text())
        assert len(tape["events"]) == 1, f"D4c-T1 record must hold exactly one decision event: {pid}"
        ...
        for ev in tape["events"]:
            assert ev["decision_kind"] == "TARGET_SELECTION", "D4c-T1 decision must be TARGET_SELECTION"
    ...
    assert decision_events == 4, "D4c-T1 must evidence exactly 4 TARGET_SELECTION events"
    gate = {
    ...
    print(f"WS33_ABC_D4C_CERTIFY=PASS paths=4 decisions={decision_events} rng={rng_events}")
    return 0
