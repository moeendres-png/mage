#!/usr/bin/env python3
"""WS33-D D4b certifier: adjudicate the emblem campaign evidence gate.

Verifies markers, campaign index, empty diagnostics, per-record assertion
results (including command-zone emblem assertions and the engine-owned
entry-doubling proof), decision-tape ACCEPTED-only validation, RNG replay
service, hidden-observation PASS, replay divergence 0, and source/pin/model
binding. Emits the D4b gate JSON. Any violation fails closed (nonzero exit).
"""
...
def main() -> int:
    ...
    assert plan["provable_count"] == 3 and len(plan["provable_ids"]) == 3
    assert plan.get("batch") == "D4b"
    ...
    for name in ("emblem-record-diagnostics.jsonl", "emblem-replay-diagnostics.jsonl"):
        p = root / name
        assert p.is_file() and p.stat().st_size == 0, f"diagnostics not empty: {name}"
    index = json.loads((root / "campaign-index.json").read_text())
    assert len(index["records"]) == 3, "campaign index must admit 3 records"
    ...
    print(f"WS33_ABC_D4B_CERTIFY=PASS paths=3 decisions={decision_events} rng={rng_events}")
    return 0
