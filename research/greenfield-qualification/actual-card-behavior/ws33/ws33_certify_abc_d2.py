#!/usr/bin/env python3
"""WS33-D D2 certifier: adjudicate the lifelose campaign evidence gate.
...
Emits the D2 gate JSON. Any violation fails closed (nonzero exit).
"""
...
def main() -> int:
    ...
    for name in ("lifelose-record-diagnostics.jsonl", "lifelose-replay-diagnostics.jsonl"):
        p = root / name
    ...
    gate = {
        "schema": "commander-simulator-next.ws33d-d2-gate.v1",
        "batch": "D2",
    ...
    print(f"WS33_ABC_D2_CERTIFY=PASS paths=8 decisions={decision_events} rng={rng_events}")
    return 0
