#!/usr/bin/env python3
"""WS33-D D1 certifier: adjudicate the lifegain campaign evidence gate.

Verifies markers, campaign index, empty diagnostics, per-record assertion
results, decision-tape ACCEPTED-only validation, RNG replay service,
hidden-observation PASS, replay divergence 0, and source/pin/model binding.
Emits the D1 gate JSON. Any violation fails closed (nonzero exit).
"""
...
def main() -> int:
    ...
    assert plan["provable_count"] == 10 and len(plan["provable_ids"]) == 10
    deferred = json.loads(a.deferred.read_text())
    assert len(deferred) == 12
    root = a.campaign_root

    for name in ("lifegain-record-diagnostics.jsonl", "lifegain-replay-diagnostics.jsonl"):
        p = root / name
        assert p.is_file() and p.stat().st_size == 0, f"diagnostics not empty: {name}"
    index = json.loads((root / "campaign-index.json").read_text())
    assert len(index["records"]) == 10, "campaign index must admit 10 records"
    ...
    gate = {
        "schema": "commander-simulator-next.ws33d-d1-gate.v1",
        "batch": "D1",
    ...
    print(f"WS33_ABC_D1_CERTIFY=PASS paths=10 decisions={decision_events} rng={rng_events}")
    return 0
