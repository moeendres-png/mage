...
from qualify import SECRET_MARKERS, UNBOUND_PRODUCTION_CATEGORIES, execute_witness, load_contract, qualify
...
class FailureSemanticsContractTest(unittest.TestCase):
    ...
    def test_integrated_gate_stays_fail_closed_without_actual_adapters(self):
        gate = qualify("test-head", "test-tree", java_contract_pass=True, q1_validator_pass=True)
        self.assertEqual("FAIL_INCOMPLETE", gate["FAILURE_SEMANTICS"])
        self.assertIsNone(gate["production_reachable_untyped_failure_outcomes"])
        self.assertIsNone(gate["production_reachable_fallback_failure_handling"])
        self.assertEqual(UNBOUND_PRODUCTION_CATEGORIES, set(gate["unbound_production_categories"]))
        self.assertFalse(gate["hard_gate_values"]["ALL_PRODUCTION_REACHABLE_CATEGORIES_BOUND_TO_ACTUAL_ADAPTERS"])
